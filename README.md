**FOR GRADER READING FINAL SUBMIT**
**-------------------------------**
The layout of the project has not been changed much from the default layout. There were almost no modifications made to frontier and worker.py besides accounting for server crashes.

There is a report file (report.txt) with that contains answers to questions asked in the assignment, namely:
...
**4.** How many subdomains did you find in the ics.uci.edu domain? Submit the list of subdomains ordered alphabetically and the number of unique pages detected in each subdomain. The content of this list should be lines containing URL, number
